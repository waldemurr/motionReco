...
//Initialize net
void RnnNet::initNet(std::vector<int> layer_neuron_num_)
{
...
void RnnNet::initWeight(cv::Mat &dst, int type, double a, double b)
{
    if (type == 0)
    {
        randn(dst, a, b);
...
void RnnNet::initWeights(int type, double a, double b)
{
...
void RnnNet::initBias(cv::Scalar& bias_)
{
...
void RnnNet::train(cv::Mat input, cv::Mat target_, float loss_threshold, bool draw_loss_curve)
{
    if (input.empty())
    {
        std::cout << "Input is empty!" << std::endl;
...
//Test
void RnnNet::test(cv::Mat &input, cv::Mat &target_)
{
    if (input.empty())
    {
        std::cout << "Input is empty!" << std::endl;
...
//Predict
int RnnNet::predict_one(cv::Mat &input)
{
    if (input.empty())
    {
        std::cout << "Input is empty!" << std::endl;
...
std::vector<int> RnnNet::predict(cv::Mat &input)
{
...
//Load model;
void RnnNet::load(std::string filename)
{
    cv::FileStorage fs;
